    ...
    if (l + rlen > end)
    {
      if (l != end)
      {
        ...
        ut_a(MY_AES_OK == encryption_crypt(l, len, tmp, &len,
                                           ...
                                           LOG_DEFAULT_ENCRYPTION_KEY,
                                           info.key_version));
        ut_ad(len == static_cast<uint>(end - l));
        memcpy(l, tmp, end - l);
        rlen-= end - l;
      }
      return rlen;
...
ATTRIBUTE_NOINLINE void mtr_t::encrypt()
{
  ...
  ut_ad(m_commit_lsn);
  byte tmp[mtr_buf_t::MAX_DATA_SIZE];
  mach_write_to_8(iv, m_commit_lsn);
  size_t size= 0;
  m_crc= 0;

  m_log.for_each_block([this,&size,&tmp,&iv](mtr_buf_t::block_t *b)
  {
    ut_ad(b->used() <= sizeof tmp);
    if (!size)
      size= log_encrypt_buf(iv, tmp, b->begin(), b->end());
    else
    {
      const size_t s= std::min(size, b->used());
      uint len= static_cast<uint>(s);
      ut_a(MY_AES_OK == encryption_crypt(b->begin(), len, tmp, &len,
                                         info.crypt_key, MY_AES_BLOCK_SIZE,
                                         iv, MY_AES_BLOCK_SIZE,
                                         ENCRYPTION_FLAG_ENCRYPT |
                                         ENCRYPTION_FLAG_NOPAD,
                                         ...
                                         info.key_version));
      ut_ad(len == s);
      ::memcpy(b->begin(), tmp, s);
      size-= s;
      if (!size)
        size= log_encrypt_buf(iv, tmp, b->begin() + s, b->end());
    }
