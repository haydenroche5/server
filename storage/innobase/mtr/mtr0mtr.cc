...
lsn_t mtr_t::commit_files(lsn_t checkpoint_lsn)
{
  ...
  if (log_sys.log.format == log_t::FORMAT_ENC_10_8)
  {
...
inline lsn_t log_t::append_prepare(size_t size) noexcept
{
  ...
  srv_stats.log_write_requests.inc(); // FIXME: normal variable
  ...
  size= (4 * OS_FILE_LOG_BLOCK_SIZE) - size + srv_log_buffer_size +
...
inline std::pair<lsn_t,mtr_t::page_flush_ahead> mtr_t::finish_write(size_t len)
{
  ...
  byte *seq= &log_sys.buf[log_sys.buf_free++];
  mach_write_to_4(&log_sys.buf[log_sys.buf_free], m_crc);
  log_sys.buf_free+= 4;
  ...
  *seq= log_sys.get_sequence_bit(m_commit_lsn);
