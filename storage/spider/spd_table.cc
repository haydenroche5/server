...
/*
  Overwrite a string list attribute of SPIDER_SHARE by a given engine-defined
  option.
*/
#define SPIDER_OVERRWRITE_PARAM_STR_LIST(title_name, option_name, param_name) \
...
/*
  Check if both DSN parameter and engine-defined attribute are specified that
  are for the same SPIDER_SHARE attribute. If both are specified, the function
  returns error.
*/
int spider_check_mixed_connection_spec(char *tmp_ptr, int title_length,
                                       ha_table_option_struct *option_struct)
{
  switch (title_length)
  {
  case 3:
    if ((option_struct->remote_server &&
         !strncasecmp(tmp_ptr, "srv", title_length)) ||
        (option_struct->remote_table &&
         !strncasecmp(tmp_ptr, "tbl", title_length)))
    {
      return ER_SPIDER_INVALID_CONNECT_INFO_NUM;
    }
    break;
  case 5:
    if (option_struct->remote_table &&
        !strncasecmp(tmp_ptr, "table", title_length))
    {
      return ER_SPIDER_INVALID_CONNECT_INFO_NUM;
    }
    break;
  case 6:
    if (option_struct->remote_server &&
        !strncasecmp(tmp_ptr, "server", title_length))
    {
      return ER_SPIDER_INVALID_CONNECT_INFO_NUM;
    }
    break;
  case 8:
    if (option_struct->remote_server &&
        !strncasecmp(tmp_ptr, "database", title_length))
    {
      return ER_SPIDER_INVALID_CONNECT_INFO_NUM;
    }
    break;
...
int spider_parse_connect_info(
...
#endif
  {
    ...
    while (sprit_ptr)
    {
      ...
      if ((error_num= spider_check_mixed_connection_spec(tmp_ptr, title_length,
                                                         option_struct)))
        goto error;
  ...
  SPIDER_OVERRWRITE_PARAM_STR_LIST("server", remote_server, server_names);
  SPIDER_OVERRWRITE_PARAM_STR_LIST("database", remote_database, tgt_dbs);
  SPIDER_OVERRWRITE_PARAM_STR_LIST("table", remote_table, tgt_table_names);
